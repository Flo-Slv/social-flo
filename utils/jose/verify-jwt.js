...
const verifyJwt = async (jwt, from, res = null) => {
  try {
    ...
    if (currentDate > payload.exp && from === "layout") return Boolean(false);

    // Return `currentUser` for "layout" and "middlewareAdmin".
    if (from === "layout" || from === "middlewareAdmin") return payload;

    // Return Boolean for "middleware" and "api".
    return Boolean(true);
  } catch (err) {
